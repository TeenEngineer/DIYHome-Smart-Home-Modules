...
int8_t fig = 0, ang = 0, pos = newWIDTH / 2, height = newHEIGHT - 1;
...
void drawPixelXYRotated(uint8_t x, uint8_t y, uint32_t color) {
#if MATRIX_ROTATION == 0
  drawPixelXY(x, y, color);
#elif MATRIX_ROTATION == 1  // 90° clockwise: new_x = old_y, new_y = WIDTH-1 - old_x
  drawPixelXY(y, newWIDTH - 1 - x, color);
#elif MATRIX_ROTATION == 2  // 180°
  drawPixelXY(newWIDTH - 1 - x, newHEIGHT - 1 - y, color);
#elif MATRIX_ROTATION == 3  // 270° clockwise (90° counterclockwise)
  drawPixelXY(newHEIGHT - 1 - y, x, color);
#endif
}

uint32_t getPixColorXYRotated(uint8_t x, uint8_t y) {
#if MATRIX_ROTATION == 0
  return getPixColorXY(x, y);
#elif MATRIX_ROTATION == 1
  return getPixColorXY(y, newWIDTH - 1 - x);
#elif MATRIX_ROTATION == 2
  return getPixColorXY(newWIDTH - 1 - x, newHEIGHT - 1 - y);
#elif MATRIX_ROTATION == 3
  return getPixColorXY(newHEIGHT - 1 - y, x);
#endif
}
  ...
  */
  ...
  if (gameTimer.isReady()) {        // главный таймер игры
    ...
    if (!checkArea(0)) {            // проверяем столкновение с другими фигурами
      if (height >= newHEIGHT - 2) {   // проиграл по высоте
...
void checkAndClear() {
  ...
  while (linesToClear != 0) {       // чисти чисти пока не будет чисто!
    ...
    for (byte Y = 0; Y < newHEIGHT; Y++) {   // сканируем по высоте
      full_flag = true;                   // поднимаем флаг. Будет сброшен, если найдём чёрный пиксель
      for (byte X = 0; X < newWIDTH; X++) {  // проходимся по строкам
        if ((long)getPixColorXYRotated(X, Y) == (long)0x000000) {  // если хоть один пиксель чёрный
    ...
    if (linesToClear > 0) {             // если найденных полных строк больше 1
      ...
      for (byte X = 0; X < newWIDTH; X++) {
        for (byte i = 0; i < linesToClear; i++) {//getPixColorXY(y, newWIDTH - 1 - x);
          //leds[getPixelNumber(X, lineNum + i)] = CHSV(0, 0, 255);         // закрашиваем его белым
          leds[getPixelNumber(lineNum + i, newWIDTH-1-X)] = CHSV(0, 0, 255);         // закрашиваем его белым
        }
        FastLED.show();
      ...
      for (byte val = 0; val <= 30; val++) {
        for (byte X = 0; X < newWIDTH; X++) {
          for (byte i = 0; i < linesToClear; i++) {
            ...
            leds[getPixelNumber(lineNum + i, newWIDTH-1-X)] = CHSV(0, 0, 240 - 8 * val);  // гасим белый цвет
          }
        }
        FastLED.show();
      ...
      for (byte i = 0; i < linesToClear; i++) {
        for (byte Y = lineNum; Y < newHEIGHT - 1; Y++) {
          for (byte X = 0; X < newWIDTH; X++) {
            drawPixelXYRotated(X, Y, getPixColorXYRotated(X, Y + 1));      // сдвигаем вниз
          }
          FastLED.show();
        }
        delay(10);       // задержка между "сдвигами" всех пикселей на один уровень
      }
    }
  }
  gameTimer.reset();
...
void newGameTetris() {
  ...
  delay(10);
  buttons = 4;
  height = newHEIGHT;    // высота = высоте матрицы
  pos = newWIDTH / 2;    // фигура появляется в середине
  ...
  if (gameDemo) pos = random(1, newWIDTH - 1);
...
boolean checkArea(int8_t check_type) {
  ...
  for (byte i = 0; i < 4; i++) {
    ...
    if (check_type == 1 || check_type == 3) {
      if (X + 1 > newWIDTH - 1) flag = false;    // смотрим следующий справа
      uint32_t getColor;
      if (Y < newHEIGHT)
        getColor = getPixColorXYRotated(X + offset, Y);
    ...
    if (check_type == 0 || check_type == 3) {
      uint32_t getColor;
      if (Y < newHEIGHT) {
        getColor = getPixColorXYRotated(X, Y - 1);
        if (getColor != color && getColor != 0x000000) {
          flag = false;         // если не СВОЙ цвет и не чёрный
        }
      }
...
void drawFigure(byte figure, byte angle, byte x, byte y, uint32_t color) {
  drawPixelXYRotated(x, y, color);         // рисуем точку начала координат фигуры
  ...
  for (byte i = 0; i < 3; i++) {    // рисуем 4 точки фигуры
    ...
    if (Y > newHEIGHT - 1) continue;   // если выходим за пределы поля, пропустить отрисовку
    drawPixelXYRotated(X, Y, color);
  }
}
